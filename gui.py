...
# Görünüm/ayraçlar
PRETTY_CASE_SEPARATOR = "*" * 42  # benzer vakalar arasında
HEADER_SEPARATOR      = "." * 80  # updated block için (mevcut davranışı koruyoruz)
...
# =========================
# Metin düzenleme (prettify) yardımcıları
# =========================
def prettify_markdown(md: str) -> str:
    """Başlıklar ve bloklar arası boşlukları düzenler, çoklu boşlukları azaltır."""
    if not md:
        return md
    lines = md.splitlines()
    out = []
    prev_blank = False
    for ln in lines:
        # trailing whitespaces temizle
        ln = ln.rstrip()

        # '### ' başlıklarından önce en az 1 boş satır bırak
        if ln.startswith("### "):
        ...
        # üçten fazla ardışık boşluğu tek boşluğa indir
        if ln.strip() == "":
            if not prev_blank:
                out.append("")
            prev_blank = True
    ...
    # başta/sonda gereksiz boşlukları kırp
    text = "\n".join(out).strip() + "\n"
    return text

def apply_prettify_to_text_widget(widget: tk.Text):
    """Text widget içeriğini prettify edip geri yazar."""
    content = widget.get("1.0", tk.END).rstrip()
    pretty = prettify_markdown(content)
    widget.configure(state="normal")
    widget.delete("1.0", tk.END)
    widget.insert(tk.END, pretty)
    widget.see(tk.END)
# =========================
...
def apply_classification():
    ...
    apply_prettify_to_text_widget(report_output)
...
def run_analysis():
    ...
    def _work():
        ...
        result = prettify_markdown(result.strip())
        original_report_text = result
...
"""
        try:
            ...
            latest_feedback_result = prettify_markdown(resp.choices[0].message.content.strip())
...
def add_to_report():
    ...
    block = (
        ...
        prettify_markdown(latest_feedback_result) + "\n"
    )
    report_output.insert(tk.END, block)
    apply_prettify_to_text_widget(report_output)
...
def save_updated_to_db():
    ...
    try:
        db_insert_report(
            ...
            method=method_var.get(), language=lang_var.get(), full_markdown=prettify_markdown(latest_feedback_result)
        )
...
def search_similar_action():
    ...
    # UI donmasın: butonu pasifleştir, thread'te çalıştır
    try:
        search_btn.configure(state="disabled")
    except Exception:
        pass
    set_status("Searching similar cases in local DB...")
        ...
        try:
            ...
            if not results:
                def _nores():
                    ...
                    try: search_btn.configure(state="normal")
                    except Exception: pass
                    set_status("No similar cases found.")
                report_output.after(0, _nores)
                return
            ...
            # En az 5 boş satır + başlık + her vaka arasında PRETTY_CASE_SEPARATOR
            chunks = []
            header = "\n" * 5 + "### Comparable Cases (Local DB — Summary + 7/30 days, with rationale)\n"
            chunks.append(header)

            for idx, (combo, score, exp, it) in enumerate(rows, start=1):
                ...
                chunks.append(case_block)
                if idx < len(rows):
                    chunks.append(PRETTY_CASE_SEPARATOR + "\n")
...
def export_pdf():
    ...
    try:
        ...
        secs = parse_markdown_to_sections(prettify_markdown(body)) or [("Report", body)]
...
def export_excel():
    ...
    try:
        ...
        sections = parse_markdown_to_sections(prettify_markdown(output_text_clean))
...
def about():
    messagebox.showinfo("About",
        ...
        "• Local SQLite DB (latest-only), Similar cases (Summary+7/30 with rationale, threaded)\n"
        "• Export PDF/Excel, English/Français\n"
        "• ICAO/TC aviation style\n"
        "• Clean report layout (prettified headings/spaces)\n")
